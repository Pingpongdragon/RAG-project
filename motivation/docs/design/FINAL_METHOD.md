...
> - [UNIFICATION.md](UNIFICATION.md) — `updator/` 与 `motivation/` 两套代码如何统一到 DRYAD
...
>
> 设计依据：文献调研（[REFERENCES.md](REFERENCES.md)）+ 三级 audit 实证 + 两处既有代码（`updator/qarc`、`motivation_2`）。
...
> 代码：`updator/qarc/detection/drift_detector.py::DriftLensDetector`。
...
代码：`updator/qarc/decision/kb_agent.py::KBUpdateAgent.decide`。
...
当前代码现实：`motivation_2/strategies.py` 已有 `DRYAD` 类（R1+R3+轻量 gap 检测），`updator/qarc` 有真 FID 检测器但未打通。

| 步骤 | 内容 | 状态 |
|---|---|---|
| S1 | doc 层 R1+R3（`RoutedCache`/`DRYAD` 类）+ 接入 run.py | ✅ 已做，已跑出初步数 |
| S2 | **把真 alignment-FID 检测器从 `updator/qarc` 移植进 motivation `DRYAD` 模块①**（λ·B 接缝已存在）→ 让"对齐 FID"成为论文流水线里真实生效的组件，而非 gap 占位 | ⬜ 最高杠杆 |
