"""Configuration for the canonical SupportFlow algorithm."""
from dataclasses import dataclass


@dataclass(frozen=True)
class SupportFlowConfig:
    """Detector-free support-cache configuration.

    The defaults are design-level settings, not per-dataset tuned constants.
    When entity metadata is available, SupportFlow expects two support slots so
