...
> 检测器 baseline（NoDetector/ADWIN/MMD）已实现于 `algorithms/qarc/detection/baseline_detectors.py`。
...
| **drift 设定** | **理论假设 query 来自固定分布 P(q\|Θ)**（§3）；streaming 实验（Fig2b）只是 warm-up 收敛，**非分布漂移** | 核心即 **query distribution shift over time**（sudden/gradual/cyclic）+ adaptation-speed 指标 | 🟢 **强（升级，原 🟡）**：ARC 的固定分布假设与 DRYAD 的漂移设定直接对立 |
...
> ✅ **原 §6 对"数据集撞车"的担忧已解除**：ARC 全程单跳，DRYAD 在 2Wiki/MuSiQue/HotpotQA 的
> bridge 增益是 ARC 评测**根本覆盖不到**的场景——它机制上够不到、评测里也没测多跳。两者天然在不同赛道。
>
> ⚠️ 但反过来：**不能简单 claim"我们在 ARC 的数据集上更好"**。正确做法是在 DRYAD 的多跳集上把
...
- [x] 增量 or 重建？→ **每 query 增量**（Algorithm 1）。**无预算/替换比例概念**，超容量逐项逐出。→ DRYAD 的 λ·B 是独有差异（已补进 §3）。
...
- [x] 单层 or 含 memory 层？→ **纯单层 doc 缓存**。

**D. 实验（决定能否同台比 + 借鉴数据集）**
- [x] 三个数据集 → **SQuAD / MMLU / AdversarialQA**（全单跳，与多跳三件套不撞，见 §6）。
- [x] baseline → **LFU / FIFO / Proximity / GPTCache** + ARC(w/o hubness) 自消融。DRYAD 的 baseline 池已覆盖且更广。
...
**E. 回填结论**
- [x] §3 强度表已更新：漂移处理🟢、更新预算🟢(新增)、bridge🟢、drift 设定🟡→🟢、双层🟡。
- [x] 未发现 ARC 抢占任何 DRYAD 差异点；反而其固定分布假设与单跳评测进一步凸显 DRYAD 的漂移+bridge 定位。
