...
> - `algorithms/qarc/`：完整的**漂移检测（DriftLens FID）+ Agent 决策 + 子模策展**流水线 → 提供 DRYAD 的「检测 + 决策」骨架。
...
>
> 上游素材：[STORYLINE_v1.md](../../motivation/docs/narrative/STORYLINE_v1.md)、[DESIGN_DIRECTIONS.md](DESIGN_DIRECTIONS.md)（gap G1–G5）、
> [algorithms/README.md](../../algorithms/README.md)、两处代码对照见 [UNIFICATION.md](UNIFICATION.md)。
...
┌─────────────────────────────────────────────────────────────────────┐
│ 模块 ①  DETECT — 何时该更新？                                          │
│   DriftLens FID(对齐特征) + AlignmentGap   ← 来自 algorithms/qarc         │
...
│   warmup + cooldown + 连续漂移重校准      ← 来自 algorithms/qarc          │
...
**设计主张**：检测/决策（模块①②）回答"何时、多大力度更新"，是 `algorithms/qarc` 已实现的；
...
## 3. 模块 ① DETECT — 漂移检测（来自 algorithms/qarc/detection）
...
代码：[`algorithms/qarc/detection/drift_detector.py`](../../algorithms/qarc/detection/drift_detector.py) `DriftLensDetector`。

---

## 4. 模块 ② DECIDE — Agent 决策（来自 algorithms/qarc/decision）
...
代码：[`algorithms/qarc/decision/kb_agent.py`](../../algorithms/qarc/decision/kb_agent.py) `KBUpdateAgent.decide`。
...
代码：[`motivation_2/strategies.py`](../../motivation/motivation_2/strategies.py) `QueryDriven`(R1+gate) + `RoutedCache`(R3)。
...
- **与 algorithms baseline 比较**：DRYAD vs ComRAG / ERASE / Static / Random（algorithms/base.py 统一接口），证明完整框架优于既有 KB 更新范式。
...
| DRYAD 术语 | motivation 代码 | algorithms/qarc 代码 |
|---|---|---|
...
| 模块③ R3 bridge | `RoutedCache` entity chain | （无，DRYAD 新增到 algorithms 一侧） |
